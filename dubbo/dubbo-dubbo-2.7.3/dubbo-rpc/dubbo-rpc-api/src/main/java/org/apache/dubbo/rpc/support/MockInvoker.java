...
final public class MockInvoker<T> implements Invoker<T> {
    ...
    public Result invoke(Invocation invocation) throws RpcException {
        ...
        mock = normalizeMock(URL.decode(mock));   // 规范化mock值
        if (mock.startsWith(RETURN_PREFIX)) {
            mock = mock.substring(RETURN_PREFIX.length()).trim();  // 去掉了return，剩下null
            try {
    ...
    private Invoker<T> getInvoker(String mockService) {
        ...
        T mockObject = (T) getMockObject(mockService, serviceType);  // 找到本地Mock类，并实例化
    ...
    public static Object getMockObject(String mockService, Class serviceType) {
        ...
        try {
            return mockClass.newInstance();   // 实例化
    ...
    public static String normalizeMock(String mock) {
        ...
        if (RETURN_KEY.equalsIgnoreCase(mock)) {
            return RETURN_PREFIX + "null";   // "returnnull"
        }

        if (ConfigUtils.isDefault(mock) || "fail".equalsIgnoreCase(mock) || "force".equalsIgnoreCase(mock)) {
            return "default";
        }
        // 若mock以fail:开头，则mock最终值为fail:后的字符串
        if (mock.startsWith(FAIL_PREFIX)) {
            mock = mock.substring(FAIL_PREFIX.length()).trim();
        }
        // 若mock以force:开头，则mock最终值为force:后的字符串
