...
public class InjvmProtocol extends AbstractProtocol implements Protocol {
    ...
    public boolean isInjvmRefer(URL url) {
        // 获取<dubbo:reference/>的scope的值
        String scope = url.getParameter(SCOPE_KEY);
        // Since injvm protocol is configured explicitly, we don't need to set any extra flag, use normal refer process.
        // 若scope值为local，或者 url中injvm值为true，则说明是本地injvm调用
        if (SCOPE_LOCAL.equals(scope) || (url.getParameter(LOCAL_PROTOCOL, false))) {
            ...
            return false;
            // 若generic属性为true，则不是本地调用
            // generic称为泛化调用，其会消费GenericService的实例
        } else if (url.getParameter(GENERIC_KEY, false)) {
            // generic invocation is not local reference
            return false;
            // 若exporterMap中不存在该暴露对象，则为本地调用
