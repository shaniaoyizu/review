...
public class ReferenceConfig<T> extends AbstractReferenceConfig {
    ...
    public synchronized T get() {
        // 将与<dubbo:reference/>相关的标签配置解析并合并到<dubbo:reference/>解析对象中
        checkAndUpdateSubConfigs();
        // 若要消费的提供者被销毁了，则抛出异常
        if (destroyed) {
            throw new IllegalStateException("The invoker of ReferenceConfig(" + url + ") has already destroyed!");
        }
        // 若消费者代理对象为null，则执行init()
        if (ref == null) {
            init();
        }
        return ref;
    ...
    private T createProxy(Map<String, String> map) {
        ...
        if (shouldJvmRefer(map)) {
            ...
            // 处理用户通过<dubbo:reference/>的url属性直接指定要消费的URL
            ...
            if (urls.size() == 1) {
                // 根据消费者URL获取到其所要调用的提供者代理对象invoker
    ...
    protected boolean shouldJvmRefer(Map<String, String> map) {
        ...
        boolean isJvmRefer;
        // 获取<dubbo:reference/>中的injvm属性值，判断是否为null
        if (isInjvm() == null) {
            // if a url is specified, don't do local reference
            // 若<dubbo:reference/>中的url属性不为null，则说明不是本地调用
