    ...
    protected List<URL> loadRegistries(boolean provider) {
        ...
        if (CollectionUtils.isNotEmpty(registries)) {
            ...
            for (RegistryConfig config : registries) {
                ...
                // 若<dubbo:registry/>中没有指定address，则address取0.0.0.0
                // 表示可以与任意IP相匹配
                ...
                if (!RegistryConfig.NO_AVAILABLE.equalsIgnoreCase(address)) {
                    // 定义一个map，该map中要写入的数据全部是将来组装URL的标签属性
                    Map<String, String> map = new HashMap<String, String>();
                    // 将<dubbo:application/>标签属性写入到map
                    appendParameters(map, application);
                    // 将<dubbo:registry/>标签属性写入到map
                    appendParameters(map, config);

                    map.put(PATH_KEY, RegistryService.class.getName());
                    appendRuntimeParameters(map);
                    // 若没有指定protocol属性，则取默认值dubbo
                    if (!map.containsKey(PROTOCOL_KEY)) {
                        map.put(PROTOCOL_KEY, DUBBO_PROTOCOL);
                    }
                    // 将一个address中指定的多个注册中心解析出来，并形成多个URL
                    List<URL> urls = UrlUtils.parseURLs(address, map);

                    // 格式化注册中心URL，使URL均以registry://开头，后面会添加一个属性
                    // registry=xxx，xxx为原协议，例如zookeeper
                    for (URL url : urls) {
                        url = URLBuilder.from(url)
                                .addParameter(REGISTRY_KEY, url.getProtocol())
                                .setProtocol(REGISTRY_PROTOCOL)
                                .build();
                        // 若当前为provider，且<dubbo:service/>中的register属性为true
                        // 或当前不为provider，且<dubbo:registry/>的subscribe属性为true，则记录该URL
